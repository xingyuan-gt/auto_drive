...
def test_add_and_list_car(simulation):
...
def test_head_on_collision(simulation):
    simulation.add_car("A", 2, 2, "E", "F")
    simulation.add_car("B", 3, 3, "S", "F")
    result = simulation.run_all()
...
def test_uneven_commands(simulation):
    simulation.add_car("A", 0, 0, "N", "FF")  # should end at (0, 2)
    simulation.add_car("B", 1, 1, "N", "F")   # should end at (1, 2) after 1 step
    ...
    assert pos_A == (0, 2, "N")
    assert pos_B == (1, 2, "N")  # stays at y=2 after running out of commands

    # A should move again after B has finished
    assert simulation.cars["A"].posture()[1] > simulation.cars["B"].posture()[1] or simulation.cars["A"].posture()[0] != simulation.cars["B"].posture()[0]
